...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('  JMW ACCESSORIES - IMAGE UPLOAD'))
        self.stdout.write(self.style.SUCCESS('=' * 80))
    ...
    def _download_image(self, url):
        ...
        if self._is_cloudinary_url(url):
            ...
            if public_id:
                ...
                self.stdout.write(self.style.WARNING('    ⚠ Invalid Cloudinary URL format, downloading instead'))
                # Fall through to normal download
        
        # Validate URL length
        if len(url) > 500:
            self.stdout.write(self.style.WARNING(f'    ⚠ URL too long (>{500} chars), skipping download'))
            return None
        ...
        try:
            ...
            if 'image' not in content_type:
                self.stdout.write(self.style.WARNING(f'    ⚠ Not an image (content-type: {content_type})'))
                return None
