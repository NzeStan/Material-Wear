...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
                ...
                try:
                    with transaction.atomic():
                        if product_type == 'nysc_kit':
                            result = self._process_nysc_kit(row, dry_run, skip_existing, update_existing)
                        elif product_type == 'nysc_tour':
                            result = self._process_nysc_tour(row, dry_run, skip_existing, update_existing)
                        else:
                            result = self._process_church(row, dry_run, skip_existing, update_existing)
            ...
            if update_count: self.stdout.write(self.style.SUCCESS(f'✓ Updated: {update_count}'))
            if skip_count: self.stdout.write(self.style.NOTICE(f'⊘ Skipped: {skip_count}'))
            if error_count:
    ...
    def _validate_headers(self, headers, product_type):
        ...
        if product_type == 'nysc_kit': required.append('type')
        if product_type == 'church': required.append('church')
    ...
    def _validate_category(self, name, ptype):
        ...
        cmap = {'NYSC KIT': 'nysc_kit', 'NYSC TOUR': 'nysc_tour', 'CHURCH PROG': 'church'}
    ...
    def _validate_price(self, price_str):
        try:
            ...
            if price > 9999999.99: raise ValueError('Price too large')
            return price
        except (InvalidOperation, ValueError) as e:
            raise ValueError(f'Invalid price: {str(e)}')

    def _get_or_create_category(self, name, ptype, desc=None):
        ptype_map = {'nysc_kit': 'nysc_kit', 'nysc_tour': 'nysc_tour', 'church': 'church'}
        ...
        if not desc:
            desc_map = {
                ...
                'CHURCH PROG': 'Quality church merchandise and program materials. Official church shirts, jackets, and polos for various denominations.',
            }
            desc = desc_map.get(name, f'{name} products')
        
        cat, created = Category.objects.get_or_create(
            name=name,
            defaults={'slug': slugify(name), 'product_type': ptype_map[ptype], 'description': desc}
        )
    ...
    def _download_image(self, url):
        ...
        try:
            ...
            if not any(fname.lower().endswith(e) for e in ['.jpg','.jpeg','.png','.gif','.webp']):
                fname += '.jpg'
    ...
    def _str_to_bool(self, val):
        ...
        if not val or str(val).strip() == '': return True
    ...
    def _process_nysc_kit(self, row, dry_run, skip_ex, update_ex):
        ...
        if existing:
            if skip_ex: return 'skipped'
            if not update_ex: raise ValueError('Already exists')
        ...
        data = {
            'name': name, 'type': kit_type, 'category': category, 'price': price,
        ...
        for img in ['image','image_1','image_2','image_3']:
            if img in row and row[img].strip():
                ...
                if imgf: setattr(product, img, imgf)
    ...
    def _process_church(self, row, dry_run, skip_ex, update_ex):
        ...
        data = {
            'name': name, 'church': church, 'category': category, 'price': price,
            'description': row.get('description', '').strip(),
            'available': self._str_to_bool(row.get('available', 'True')),
            'out_of_stock': self._str_to_bool(row.get('out_of_stock', 'False')),
        }
