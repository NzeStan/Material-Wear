...
class CustomAccountAdapterTests(TestCase):
    ...
    def test_send_mail_uses_correct_from_email(self):
        ...
        with patch('accounts.adapters.render_to_string') as mock_render:
            ...
            with patch.object(self.adapter, 'get_from_email', return_value='noreply@jmw.com'):
                self.adapter.send_mail(
                    'test_template',
                    'test@example.com',
                    context
        ...
        self.assertEqual(mail.outbox[0].from_email, 'noreply@jmw.com')
    ...
    def test_send_welcome_email_with_context(self):
        ...
        self.assertEqual(context['site_name'], 'JUME MEGA WEARS')
    ...
    def test_save_user_with_commit_false(self):
        ...
        with patch.object(self.adapter, 'send_welcome_email'):
            ...
            with patch('accounts.adapters.DefaultAccountAdapter.save_user', return_value=user):
                result = self.adapter.save_user(self.request, user, mock_form, commit=False)
...
class CustomSocialAccountAdapterTests(TestCase):
    ...
    def test_save_user_marks_email_as_verified(self):
        ...
        with patch('accounts.adapters.DefaultSocialAccountAdapter.save_user', return_value=user):
    ...
    def test_username_generation_increments_correctly(self):
        ...
        User.objects.create_user(username='testuser-1', email='test2@example.com', password='pass')
        User.objects.create_user(username='testuser-2', email='test3@example.com', password='pass')
...
class AdaptersIntegrationTests(TestCase):
    ...
    def test_complete_social_login_flow_with_account_linking(self):
        ...
        email = EmailAddress.objects.get(user=existing_user, email='existing@example.com')
