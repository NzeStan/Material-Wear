...
class VerifyPaystackSignatureTests(TestCase):
    ...
    def setUp(self):
        ...
        self.test_payload = b'{"event": "charge.success", "data": {"reference": "JMW-TEST"}}'
    ...
    def test_invalid_signature_modified_payload(self):
        ...
        signature = self._generate_valid_signature(original_payload, 'sk_test_1234567890abcdef')
    ...
    def test_invalid_signature_single_char_difference(self):
        ...
        payload = b'{"event": "charge.success"}'
        valid_signature = self._generate_valid_signature(payload, 'sk_test_1234567890abcdef')
...
class SanitizePaymentLogDataTests(TestCase):
    ...
    def test_sanitize_simple_dict_with_sensitive_fields(self):
        ...
        self.assertEqual(result['reference'], 'JMW-TEST-123')
    ...
    def test_sanitize_all_sensitive_fields(self):
        ...
        sensitive_fields = [
            'authorization', 'card', 'customer', 'authorization_code',
            'card_type', 'last4', 'exp_month', 'exp_year', 'bin',
            'bank', 'channel', 'signature', 'account_name'
        ]
        
        data = {field: f'sensitive_{field}_value' for field in sensitive_fields}
        data['reference'] = 'JMW-TEST'  # Non-sensitive field
        ...
        for field in sensitive_fields:
            self.assertEqual(result[field], '[REDACTED]', 
                           f"Field '{field}' was not redacted")
        
        # Non-sensitive field should remain
        self.assertEqual(result['reference'], 'JMW-TEST')
    ...
    def test_sanitize_list_of_dicts(self):
        ...
        data = {
            'transactions': [
                {'reference': 'JMW-001', 'card_type': 'visa'},
                {'reference': 'JMW-002', 'card_type': 'mastercard'},
                {'reference': 'JMW-003', 'last4': '4081'}
            ]
        }
        
        result = sanitize_payment_log_data(data)
        
        self.assertEqual(result['transactions'][0]['reference'], 'JMW-001')
        self.assertEqual(result['transactions'][0]['card_type'], '[REDACTED]')
        self.assertEqual(result['transactions'][1]['reference'], 'JMW-002')
        self.assertEqual(result['transactions'][1]['card_type'], '[REDACTED]')
        self.assertEqual(result['transactions'][2]['reference'], 'JMW-003')
    ...
    def test_sanitize_unicode_field_names(self):
        ...
        self.assertEqual(result['référence'], 'JMW-TEST')
    ...
    def test_sanitize_very_large_dict(self):
        ...
        data = {
            'reference': 'JMW-TEST',
            'items': [
                {'id': i, 'card_type': 'visa', 'amount': 1000}
                for i in range(1000)
            ]
        }
    ...
    def test_sanitize_paystack_webhook_payload(self):
        ...
        self.assertEqual(result['data']['reference'], 'JMW-TEST-123')
...
class SecurityIntegrationTests(TestCase):
    ...
    def test_webhook_flow_signature_then_sanitize(self):
        ...
        self.assertEqual(sanitized['data']['reference'], 'JMW-TEST-123')
...
class SecurityPerformanceTests(TestCase):
    ...
    def test_sanitize_many_items_performance(self):
        ...
        self.assertEqual(result['transactions'][0]['card_type'], '[REDACTED]')
        self.assertEqual(result['transactions'][5000]['authorization_code'], '[REDACTED]')